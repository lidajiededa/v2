...
class NPUAttentionMetadataBuilder(V1AttentionMetadataBuilder[NPUMetadata]):
    ...
    def build(self,
        ...
        if (model_extra_config.operator_opt_config.kv_nz and num_prefills > 0 
            and num_decodes == 0 and num_decode_tokens == 0):
    ...
    def forward(
        ...
        if use_bsnd:
            ...
            for i in range(attn_metadata.num_decodes, attn_metadata.num_decodes + attn_metadata.num_prefills):
                ...
                attn_output = torch_npu.npu_fused_infer_attention_score(**kwargs)[0].view(-1, self.num_heads, self.head_size)
