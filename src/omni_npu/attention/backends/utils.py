...
class SPManager:
    ...
    @lazy_init
    def _scheme_cp_attn(self, cumlens: torch.Tensor, computed_lens: torch.Tensor, blk_table_ref: torch.Tensor, pg: int, tab: int):
        frag_num = self.sp_size * 2
...
class KVSPMaganer:
    ...
    def _scheme_reorg(self):
        ...
        sects = [local_section(rank, cnt, base, loc)
    ...
    def _scheme_ag(self):
        ...
        req_pages = [tab[:cnt] for tab, cnt in # for each req
            zip(self.blk_table, local_blks)]
...
def paged_scatter(
    ...
    def extraction(base: np.ndarray, lens: np.ndarray):
        return [(int(src), int(src + num)) for src, num
            in zip(base, lens) if num > 0]
