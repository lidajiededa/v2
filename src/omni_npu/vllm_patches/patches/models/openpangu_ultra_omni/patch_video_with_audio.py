...
class AsyncMultiModalContentParser(VLLMPatch):
    ...
    def parse_video(self, video_url: str | None, uuid: str | None = None) -> None:
        ...
        if use_audio_in_video:
            ...
            # Execute FFmpeg
            def _run_ffmpeg_extract(source_path: str) -> bytes:
                try:
                    audio_output, _ = (
                        ffmpeg
                        .input(source_path)
            ...
            if "input" in kwargs:
                ...
                try:
                    with tempfile.NamedTemporaryFile(delete=False, suffix=f".{safe_suffix}") as tmp:
                        ...
                        tmp.flush()
                    audio_bytes = _run_ffmpeg_extract(tmp_path)
                finally:
                ...
                audio_bytes = _run_ffmpeg_extract(video_input)
                ...
                for mm_feature in req_state.mm_features:
                    ...
                    if (is_embed := pos_info.is_embed) is not None:
                        ...
                        if start_pos == pre_start_pos:
                            mm_embeds_item = torch.zeros((len(is_embed), encoder_output.shape[1]), dtype=encoder_output.dtype, device=encoder_output.device)
                            mm_embeds_item[is_embed] = encoder_output[curr_embeds_start:curr_embeds_end]
