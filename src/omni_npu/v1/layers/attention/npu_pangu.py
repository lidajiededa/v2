...
def _get_slot_mapping_2d(attn_metadata):
...
class NPUPanguIndexer(torch.nn.Module):
    ...
    def forward(
        ...
        torch.ops.vllm.npu_pangu_indexer_cache_update(
            k,
...
class NPUPanguSparseAttention(torch.nn.Module):
    def __init__(
        ...
        self.maybe_enable_sp_for_mome = (
            ...
            not vllm_config.scheduler_config.enable_chunked_prefill and
            not vllm_config.cache_config.enable_prefix_caching
        )
        self.enable_prefill_absorb_pa = (
    ...
    def process_weights_after_loading(self) -> None:
        ...
        self.W_UV = self.W_UV.transpose(1, 2).contiguous()
        self.W_UK_T = self.W_UK.contiguous()
    ...
    def _forward_prefill(
        ...
        if self.is_dsa_layer:
            ...
            if self.enable_prefill_absorb_pa:
    ...
    def _forward_prefill_FC2(
        ...
        enable_sp_for_mome = self.maybe_enable_sp_for_mome and \
    ...
    def _mla_prolog_sequential(
        ...
        if (
            ...
            or self.enable_prefill_absorb_pa
        ):
    ...
    def _mla_prolog_swa_multistream(
        ...
        with torch.npu.npugraph_ex.scope.limit_core_num(16,24):
            ...
            if self.use_mome:
                q_lora = self._apply_MOME(
                    q_lora, self.qa_conv, 0, attn_metadata, mome_metadata,
                )
            q_lora = self.q_a_layernorm(q_lora)
    ...
    def _mla_prolog_dsa_multistream(
        ...
        """DSA decode multi-stream: main stream does Q + indexer k/w, side stream does KV + indexer q + LI.

        Main stream: q_a -> norm -> indexer_k/w -> q_b_nope/pe -> W_UK_T -> rope
        Side stream: kv_down -> [wait q_lora] -> indexer_q -> [wait k/w] -> rope -> cache_update -> LI -> kvrmsnorm
        """
        main_stream = torch.npu.current_stream()
        ...
        # Side stream: KV + indexer q + LI + kvrmsnorm
        with torch.npu.stream(self.side_stream):
            hidden_states_event.wait(self.side_stream)
