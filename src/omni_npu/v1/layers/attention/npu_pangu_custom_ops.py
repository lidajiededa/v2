...
(a) `mutates_args` must stay `[]` for ops in this file, even when the body
    ...
    sequential order with downstream consumers is preserved by the FX
    graph trace (back-to-back `torch.ops.vllm.*` calls serialize in order).
...
(b) Supported return types: `Tensor` or `None`. Do NOT use
      * `tuple[Tensor, ...]`               — infer_schema rejects tuples.
    ...
    op consumes them", split into two ops with `mutates_args=[]` and rely
    on the FX graph's call-order to serialize them.
...
def npu_pangu_swa_decode_fake(
    ...
    return torch.empty(
        (num_tokens, layer.num_local_heads * layer.v_head_dim),
...
# In-place writes into kv_cache; returns None. Ordering with the
# downstream lightning_indexer call is preserved by FX-graph trace order.
