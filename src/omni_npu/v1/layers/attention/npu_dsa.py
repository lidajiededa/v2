...
class Indexer(torch.nn.Module):
    ...
    def _li_prolog(self, x, qr, cos, sin): # -> wi, qi, ki
...
class NPUDeepseekSparseAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.noncontiguous_kv:
            self.dummy_value_cache = torch.zeros(
                (1, cache_config.block_size, 1, self.kv_lora_rank), **self.default_cfg)
    ...
    def _q_absorb(
        ...
        if absorb.size(-1) % 128 != 0 or absorb.size(-2) % 128 != 0:
            ...
            if not self.on_ascend950 and q_nope.size(1) * q_nope.size(2) < 65536:
    ...
    def _post_attn_absorb(self, out: torch.Tensor) -> torch.Tensor:
        assert out.dim() == 3
        if not self.on_ascend950 and out.size(1) * out.size(2) < 65536:
    ...
    def _apply_sink_offset(self, topk_idx: torch.Tensor):
    ...
    def _forward_prefill(
        ...
        topk_idx = self.indexer._apply_lightning_indexer(
            wi, qi, ki_cache, q_cumlens, kv_lens, block_table)
        ...
        if self.ena_sp and self.o_proj.tp_size == 1:
    ...
    def _forward_prefill_cp(
        ...
        wi, qi, ki = self.indexer._li_prolog_ext(
            sp_x, q_lora, sp_x, cp_cos_sin, sp_cos_sin) # TND
    ...
    def _forward_decode(
        ...
        # TODO: temporarily added decode-kvsp before kv-trans adapts kvsp, and not support graph
        if self.ena_kvsp:
            assert not self.use_mome and not self.noncontiguous_kv
        ...
        out = self._apply_attn_absorb(
            q_nope=q_nope, q_pe=q_pe,       # TND
            q_cumlens=q_cumlens, kv_lens=kv_lens, # [B]
    ...
    if has_prefill and has_decode:
        with sp_disabled(self, hs) as (x, y, out):
            ...
            y[d_slice] = self._forward_decode(
                x[d_slice], cos[d_slice], sin[d_slice], decode, kv_cache, pd_mixed_flag)
    elif has_prefill:
        ...
        if self.ena_cp:
            ...
            out[p_slice] = self._forward_prefill(
                hs[p_slice], cos[p_slice], sin[p_slice], prefill, kv_cache)
    else: # has_decode
        with sp_disabled(self, hs) as (x, y, out):
            y[d_slice] = self._forward_decode(
                x[d_slice], cos[d_slice], sin[d_slice], decode, kv_cache)
    return out.tensor()
