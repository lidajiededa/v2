...
model class and overriding key methods

like `__init__`, `forward`, `compute_logits`, `sample`, and `load_weights`.
...
def mock_model_class_factory(base_class: type) -> type:
    ...
    def get_unique_req_identifier(block_row):
        ...
        return block_row[
            0
        ].item()  # The first block table entry is unique for each request
