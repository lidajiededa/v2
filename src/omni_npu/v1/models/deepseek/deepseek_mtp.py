...
"""Inference-only DeepseekV3 MTP model."""
...
from omni_npu.v1.layers.vocab_parallel_embedding import NPUVocabParallelEmbedding, NPUParallelLMHead
...
class SharedHead(nn.Module):
    def __init__(
        ...
        self.head = NPUParallelLMHead(
            ...
            dp_parallel=model_extra_config.parall_config.ena_dp_lmhead_parallel,
        )
...
class DeepSeekMultiTokenPredictor(nn.Module):
    ...
    def embed_input_ids(self, input_ids: torch.Tensor) -> torch.Tensor:
        # used when has mm inputs, do not need scatter
        return self.embed_tokens(input_ids, enable_scatter=False)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids, enable_scatter=model_extra_config.parall_config.ena_seq_parallel)
        else:
