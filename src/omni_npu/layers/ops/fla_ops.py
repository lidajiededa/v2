...
def fused_recurrent_gated_delta_rule_fwd(
    ...
    S = initial_state[indices].to(torch.float32).transpose(-1,-2) #[bs, C, D, E]
    ...
    for t in range(T):
        ...
        torch_npu.npu_scatter_nd_update_(initial_state, indices.unsqueeze(1), S.to(torch.bfloat16).transpose(-1,-2))
