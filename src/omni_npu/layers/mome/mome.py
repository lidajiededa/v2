    ...
    def forward(self, hidden_states: torch.Tensor, only_prefill=False, force_decode=False, short_prefill=False) -> torch.Tensor:
        ...
        if (attn_metadata.num_prefills > 0 or not batch_descriptor.uniform or short_prefill) and not force_decode:
            ...
            for i in range(batch_size):
                ...
                conv_input = torch.cat([self.cache_states[cache_slot_id[i], :self.cache_length].contiguous(), local_input], dim=0)
                ...
                if e-s == decode_token_num:
                    self.cache_states[i + 1 + cache_idx_offset, :self.cache_capacity, :] = conv_input[-self.cache_capacity:, :]
                else:
                    self.cache_states[i + 1 + cache_idx_offset, :self.cache_length, :] = conv_input[-self.cache_length:, :]
            ...
            conv_input = torch.cat([self.cache_states[cache_slot_id[:batch_size], :self.cache_length], current_state], dim=1)
